...
//    copy_two_charArray(receiver,data[receiver_id].user_name);
    ...
    sprintf((char *) data[sender_id].trans[sender_index].trans_record, "%s%s%s%s%d", from, sender, to, receiver,1000);
    sprintf((char *) data[receiver_id].trans[receiver_index].trans_record, "%s%s%d", receive, sender,1000);
